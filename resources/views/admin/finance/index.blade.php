        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div class="bg-blue-50 rounded-xl p-4 border border-blue-100">
                ...
                <p class="text-xl font-black text-blue-700">Rp
                    {{ number_format($marginAnalysis['gross_profit'], 0, ',', '.') }}</p>
            ...
            <div
                ...
                <p class="text-xs {{ $marginAnalysis['net_profit'] >= 0 ? 'text-green-500' : 'text-red-500' }} mt-1">Margin:
                    {{ number_format($marginAnalysis['net_margin'], 1) }}%</p>
            </div>
        </div>
    </div>
                ...
                <table class="w-full">
                        ...
                        @foreach($hppByProduct as $index => $product)
                            ...
                            <tr x-show="showDetail" x-transition class="bg-amber-50">
                                <td colspan="8" class="py-3 px-4">
                                    <div class="text-sm">
                                        <p class="font-bold text-amber-800 mb-2">📦 Rincian Bahan Baku untuk 1 {{ $product['name'] }}:</p>
                                        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
                                            @foreach($product['ingredients'] as $ing)
                                                <div class="bg-white p-2 rounded-lg border border-amber-200 flex justify-between items-center">
                                                    <div>
                                                        <span class="font-medium text-gray-700">{{ $ing['name'] }}</span>
                                                        <p class="text-xs text-gray-500">{{ $ing['amount'] }} {{ $ing['unit'] }} × Rp {{ number_format($ing['cost_per_unit'], 0, ',', '.') }}</p>
                                                    </div>
                                                    <span class="font-bold text-amber-600">Rp {{ number_format($ing['total_cost'], 0, ',', '.') }}</span>
                                                </div>
                                            @endforeach
                                        </div>
                                        <div class="mt-2 pt-2 border-t border-amber-200 flex justify-between">
                                            ...
                                            <span class="font-black text-amber-700">Rp {{ number_format($product['hpp_per_unit'], 0, ',', '.') }}</span>
                                        </div>
                                    </div>
                                </td>
                            </tr>
                    ...
                    <tfoot class="bg-gray-100 border-t-2 border-gray-300">
                        <tr>
                            ...
                            <td class="py-3 px-4 text-center font-bold">{{ number_format(collect($hppByProduct)->sum('qty_sold')) }}</td>
                            <td class="py-3 px-4"></td>
                            <td class="py-3 px-4 text-right font-bold text-amber-700">Rp {{ number_format($hpp, 0, ',', '.') }}</td>
                            <td class="py-3 px-4"></td>
                            <td class="py-3 px-4"></td>
